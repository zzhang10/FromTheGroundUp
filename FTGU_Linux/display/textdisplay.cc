    ...
    for (int i = 0; i < height; i++) {
        for (int j = 0; j < width; j++) {
            if ((theDisplay[i][j] == global__displayStair) && (!hasSmartPointer)) {
                ...
                if (global__modded){
		    if (!global__hacker) out << "\033[1;30m" << global__displayNormalCell << "\033[0m";
                    else out << "\033[1;31m" << global__displayNormalCell<< "\033[0m";
                } else {
		...
		if (global__modded) {
		    if ((i == 0) || (i == height - 1) || (j == 0) ||
		        ...
		        if (theDisplay[i][j] != global__displayPlayer){
                            out << "\033[1;34m" << theDisplay[i][j] << "\033[0m";
			} else {
			    out << "\033[1;31m" << theDisplay[i][j] << "\033[0m";
			}
		    } else if(theDisplay[i][j] == global__displayGold){ 
                        out << "\033[1;33m" << global__displayGold<< "\033[0m";
                    } else if (theDisplay[i][j] == global__displayPotion){ 
                        out << "\033[1;32m" << global__displayPotion<< "\033[0m";
                    } else if (theDisplay[i][j] == global__displayPlayer){ 
                        out << "\033[1;31m" << global__displayPlayer<< "\033[0m";
	    ...
	    out << "\033[0m";
            if (i == 15){
                out <<"\033[1;35m"<< left << setfill (' ') <<setw(13) <<
                "    Enemies near you: \033[0m";
            } else if (i == 17){
                out <<elines[0];
            } else if (i == 18){
                out <<elines[1];
